...
class MQueryConverter:
    ...
    def _apply_types(self, fields: List[Dict], previous_step: str):
        ...
        Qlik-qualified names (e.g. "Model_Master.ModelID") are preserved exactly.
        They are the actual CSV column headers that Qlik writes, so Power BI must
        reference them by the same full string — both in M expressions and in the
        BIM relationship definitions.  Do NOT strip the table-prefix here.
        """
        ...
        pairs = []
        for f in typed:
            # Alias takes priority (explicit AS rename); otherwise keep full qualified name
