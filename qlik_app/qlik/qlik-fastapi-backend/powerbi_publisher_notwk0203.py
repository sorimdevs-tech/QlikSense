...
def _extract_fields_from_m(expr: str) -> list:
    ...
    type_map = {
        ...
        "datetime": "datetime", "logical": "boolean", "Int64.Type": "integer"
    }
    fields = []
    for part in cols_str.split(","):
        ...
        # If mapped to generic string, use name-based heuristic for better types
...
#     """
#     Fix M expressions where inline data values contain embedded newlines.
#     Analysis Services M engine requires all values in a row to be on one line.
#     E.g. {"EV0001", "Tata", "1550\n000"} breaks with 'Token , expected'.
#     Strategy: collapse each {...} row onto a single line.
#     """
...
#             # Row started but not finished on this line — start buffering
#             in_row = True
#             buffer = line
#         elif in_row:
#             buffer += " " + stripped
...
def _fix_multiline_rows(expr: str) -> str:
    """
    Collapse multiline data rows in #table() M expressions.
    A data row starts with { and ends with }, joining continuation lines.
    """
    lines = expr.split("\n")
    result = []
    in_row = False
    ...
    for line in lines:
        ...
        if in_row:
            # We're inside a row — append until we find the closing }
            ...
            # Check if this line starts a row but doesn't close it
            if stripped.startswith("{") and stripped.endswith(("},{", "},")):
                # Complete row on one line
                result.append(line)
            elif stripped.startswith("{") and not (stripped.endswith("},") or stripped.endswith("}")):
                # Row starts here but continues on next line(s)
                in_row = True
...
def _sanitize_m(expr: str) -> str:
    ...
    return expr
# def _sanitize_m(expr: str) -> str:
#     """Fix missing outer braces and multiline rows in M expressions."""
#     lines = expr.splitlines()
#     result = []
#     buffer = ""
#     in_row = False
...
#     fixed = []
#     past_type_table = False
#     i = 0
#     while i < len(result):
#         line = result[i]
#         stripped = line.strip()
#         if "type table" in stripped:
#             past_type_table = True
#             fixed.append(line)
#             i += 1
#             continue
#         is_bare = (past_type_table and stripped.startswith("{") and
#                    len(stripped) > 1 and stripped[1] == chr(34))
#         if is_bare:
...
#             while i < len(result):
#                 s = result[i].strip()
#                 if s.startswith("{") and len(s) > 1 and s[1] == chr(34):
...
#             nl = chr(10)
#             sep = "," + nl + pfx + "        "
#             fixed.append(pfx + "{ " + sep.join(rows) + nl + pfx + "    }")
#             continue
#         fixed.append(line)
#         i += 1
...
class _Publisher:
    ...
    def _build_bim(
        ...
        for t in tables_m:
            ...
            # If fields empty, extract column names from M expression
            if not fields:
                fields = _extract_fields_from_m(t.get("m_expression", ""))
            ...
            tmd_tables.append({
                ...
                "partitions": [{
                    ...
                    "source": {
                        "type": "m",
                        "expression": _fix_multiline_rows(_sanitize_m(expr)).splitlines()
                    }
                }]
            })
    ...
    def _deploy_via_fabric(
        ...
        try:
            ...
            if resp.status_code in (200, 201, 202):
                ...
                # 🔹 Extract dataset ID from initial 202 header (India Central fix)
                ...
                if dataset_id == "SUCCEEDED_NO_ID":
                    # Fabric doesn't return ID in poll — look up by name
