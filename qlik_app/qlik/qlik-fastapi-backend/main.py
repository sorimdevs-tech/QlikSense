...
# FIX 2: Moved ALL module-level imports to the top â€” matplotlib, numpy,
#         QlikClient, processor, table_tracker, powerbi_service_delegated,
#         powerbi_auth, pydantic â€” previously scattered mid-file after route
...
#         Three definitions â€” only first is ever called. Kept one clean version.
...
#         BEFORE that function was imported â€” runtime NameError. Fixed by
...
# FIX 10: powerbi_dataset_publisher integrated â€” /api/migration/publish-dataset
#          endpoint added, wiring parse_result â†’ publish_dataset().
# ============================================================
...
app = FastAPI(
    ...
    version="1.0.0",
    description="Extract â†’ Infer â†’ Normalize â†’ XMLA Write â†’ TOM Create â†’ ER Diagram"
)
...
# FIX 3+4: single definition of each dependency â€” no duplicates
...
# ==================== ROOT & HEALTH ====================
# FIX 5+6: single / and single /health â€” duplicates removed
...
async def download_mquery(
    ...
    try:
        logger.info(f"ðŸ“¥ Download M Query - App: {app_id}, Table: {table}")
        ...
        logger.info(f"âœ… Script fetched: {len(raw_script)} characters")

        try:
            from loadscript_parser import LoadScriptParser
            parser       = LoadScriptParser(raw_script)
            parse_result = parser.parse()
            logger.info(f"âœ… Script parsed: {parse_result.get('summary', {}).get('tables_count', 0)} tables found")
        except Exception as e:
            logger.warning(f"âš ï¸ Parse error, falling back: {e}")
            parse_result = {
        ...
        try:
            ...
            logger.error(f"âŒ Generator error: {e}")
            m_query = (
...
def apply_dax_measures(pbi_service: PowerBIService, dataset_id: str, measures: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    try:
        ...
        for measure in measures:
            try:
                print(f"  âœ“ Prepared measure: {measure['name']}")
                success_count += 1
            except Exception as e:
                print(f"  âœ— Failed to apply measure {measure['name']}: {str(e)}")
...
def create_auto_report(pbi_service: PowerBIService, dataset_id: str, dataset_name: str, columns: List[str]) -> Dict[str, Any]:
    ...
    """
    try:
        print(f"ðŸ”§ Creating auto-report for dataset: {dataset_name}")
...
def clone_template_report(pbi_service: PowerBIService, dataset_id: str, dataset_name: str, template_report_id: str) -> Dict[str, Any]:
    ...
    """
    try:
        print(f"ðŸ“‹ Cloning template report: {template_report_id}")
        ...
        print(f"âœ… Report cloned successfully!")
        ...
        print(f"âŒ Template cloning failed: {str(e)}")
        return {
            "status": "failed",
            "error": str(e)
        }
...
def create_and_publish_report(pbi_service: PowerBIService, dataset_id: str, dataset_name: str, columns: List[str]) -> Dict[str, Any]:
    ...
    """
    try:
        print(f"ðŸ› ï¸ Creating Power BI report with visualizations...")
        ...
        print(f"ðŸ“Š Creating report: {report_name}")
        response = requests.post(create_report_url, headers=headers, json=payload, timeout=30)
        
        if response.status_code not in [200, 201, 202]:
            print(f"âš ï¸ Create report failed ({response.status_code}): {response.text}")
        ...
        print(f"âŒ Report creation error: {str(e)}")
        return {
            "status": "error",
            "error": str(e)
        }
...
def create_basic_report(pbi_service: PowerBIService, dataset_id: str, dataset_name: str, columns: List[str]) -> Dict[str, Any]:
    ...
    """
    try:
        print(f"ðŸ“Š Creating visualization URL for dataset: {dataset_name}")
        ...
        print(f"âœ… Visualization URL generated!")
        ...
        print(f"âŒ URL generation failed: {str(e)}")
        return {
            "status": "failed",
            "error": str(e)
        }
...
def get_or_create_default_report(pbi_service: PowerBIService, dataset_id: str, dataset_name: str, columns: List[str]) -> str:
    ...
    try:
        ...
        if report_result.get("status") == "success":
            ...
            if report_url:
                print(f"ðŸŽ¯ Will open report: {report_url}")
                return report_url
        ...
        print(f"âš ï¸ Could not create report, using workspace URL: {str(e)}")
...
async def list_applications(
    ...
    try:
        ...
        message = str(e)
        logger.error(f"Failed to retrieve applications: {message}")
        if "authentication failed (401)" in message.lower():
            raise HTTPException(status_code=401, detail=message)
        raise HTTPException(status_code=500, detail=f"Failed to retrieve applications: {message}")
    except Exception as e:
...
# FIX 7: single /applications/{app_id}/tables â€” duplicate removed
@app.get("/applications/{app_id}/tables")
...
async def get_table_data_enhanced(
    ...
    """
    try:
        print(f"ðŸ“Š Attempting to fetch table '{table_name}' from app '{app_id}'...")
        ...
        if SCRIPT_PARSER_AVAILABLE:
            print(f"ðŸ” Trying script extraction first (for inline/CSV data)...")
            try:
                ...
                if script_result.get("success"):
                    ...
                    if script:
                        ...
                        if table_preview.get("success"):
                            ...
                            print(f"âœ… SUCCESS: Extracted {len(rows)} rows from script for table '{table_name}'")
                            return {
                                "success": True,
                            ...
                            print(f"âš ï¸ Script found but table '{table_name}' not in script: {table_preview.get('error')}")
                    else:
                        print("âš ï¸ No script found in app")
            except Exception as e:
                print(f"âš ï¸ Script extraction failed: {str(e)}")
        else:
            print("âš ï¸ Script parser not available, skipping script extraction")
        
        # STRATEGY 1: Direct fetch with provided name (hypercupe method)
        print(f"ðŸ”„ Trying hypercube method...")
        ...
        if result.get("success", False):
            ...
            if rows and len(rows) > 0:
                ...
                if not has_placeholder:
                    print(f"âœ… Successfully retrieved table data using direct name")
                    return result
                else:
                    print(f"âš ï¸ Got placeholder data, trying other strategies...")
            else:
                return result
        
        print(f"âš ï¸ Direct fetch failed or returned placeholders: {result.get('error', 'Unknown error')}")
        
        # Strategy 2: Get list of available tables and try case-insensitive match
        print(f"ðŸ” Retrieving list of available tables...")
        tables_info = ws_client.get_app_tables_simple(app_id)
        
        if tables_info.get("success", False):
            available_tables = tables_info.get("tables", [])
            print(f"ðŸ“‹ Found {len(available_tables)} tables in app")
            ...
            for table_info in available_tables:
                ...
                for name in table_names_to_try:
                    if name and name.strip().lower() == requested_lower:
                        print(f"âœ… Found matching table name: '{name}'")
                        ...
                        if result.get("success", False):
                            ...
                            if rows:
                                ...
                                if not has_placeholder:
                                    print(f"âœ… Successfully retrieved data using matched name: '{name}'")
                                    return result
                        ...
                        if SCRIPT_PARSER_AVAILABLE:
                            try:
                                ...
                                if script:
                                    ...
                                    if table_preview.get("success"):
                                        print(f"âœ… Found actual data via script with matched name: '{name}'")
                                        return {
                                            "success": True,
                                            "table_name": name,
                                            "columns": table_preview.get("columns", []),
                                ...
                                print(f"âš ï¸ Script extraction with matched name failed: {str(e)}")
            
            print(f"âš ï¸ No matching table found. Available tables: {[t.get('name', 'unknown') for t in available_tables]}")
        ...
        print(f"âŒ Error in enhanced table fetch: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to get table data: {str(e)}")
...
async def vehicle_summary(
    ...
    try:
        ...
        if table_data_result.get("success"):
            print(f"âœ… Got actual table data")
            rows = table_data_result.get("rows", [])
        ...
        if not SCRIPT_PARSER_AVAILABLE:
            print(f"âš ï¸ Script parser not available, returning metadata summary")
        ...
        if not result.get("success", False):
            print(f"âš ï¸ Script parsing failed, returning basic summary")
            return {
                "success": True,
                "summary": {
                    "Table": table_name,
        ...
        if not script:
            print(f"âš ï¸ No script found, returning basic summary")
            return {
                "success": True,
                "summary": {
                    "Table": table_name,
        ...
        if not table_data.get("success", False):
            print(f"âš ï¸ Table not found in script")
            return {
                "success": True,
                "summary": {
                    "Table": table_name,
        ...
        print(f"âŒ Error in vehicle_summary: {str(e)}")
        import traceback
        traceback.print_exc()
...
async def powerbi_login_acquire_token(request: dict):
    ...
    try:
        print("\n" + "="*60)
        print("ðŸ” STARTING SERVICE PRINCIPAL TOKEN ACQUISITION")
        print("="*60)
        
        # Perform synchronous token acquisition
        print("ðŸ“± Acquiring token via service principal...")
        result = auth.acquire_token_by_device_code(max_wait_seconds=10)
        
        if result.get("success"):
            print("\n" + "="*60)
            print("âœ… TOKEN ACQUISITION SUCCESSFUL!")
            print("="*60 + "\n")
            return {
                "success": True,
            ...
            print(f"\nâŒ Token acquisition failed: {result.get('error')}\n")
            return {
                "success": False,
        ...
        print(f"âŒ Token acquisition error: {str(e)}")
        import traceback
        traceback.print_exc()
...
async def process_for_powerbi(
    ...
    try:
        ...
        if has_dax_selected and dax_file:
            try:
                ...
                for measure in dax_measures:
                    print(f"  âœ“ Measure: {measure['name']}")
            except Exception as e:
        ...
        print(f"DEBUG: âœ… Processing formats selected in Export page - CSV: {has_csv_selected}, DAX: {has_dax_selected}")
        ...
        try:
            ...
            print(f"âœ… Dataset saved to Power BI Cloud!")
            print(f"   Dataset ID: {dataset_id}")
            print(f"   Table Name: {table_name}")
            print(f"   Rows: {len(rows)}")
            print(f"   ðŸ“Œ This data will persist in Power BI even if backend/app closes")
            ...
            print("ðŸ“Š Creating Power BI report with visualizations...")
...
async def powerbi_advanced(
    ...
    """
    try:
        print("ðŸš€ Advanced Power BI processing started...")
        ...
        print(f"ðŸ“Š CSV size: {len(csv_content)} bytes")
        print(f"ðŸ“ DAX size: {len(dax_content)} bytes")
        ...
        if dax_content:
            try:
                ...
                print(f"âœ… Parsed {len(measures_list)} measures from DAX file:")
                for m in measures_list:
                    print(f"   - {m['name']}")
            except Exception as e:
                print(f"âš ï¸ DAX parsing error: {str(e)}")
                measures_list = []
        ...
        try:
            ...
            print(f"âŒ CSV processing error: {str(e)}")
        ...
        print(f"ðŸ“Š Creating dataset: {dataset_name}")
        print(f"ðŸ“‹ Table: {table_name}")
        print(f"ðŸ“ˆ Rows: {len(rows)}, Columns: {len(columns)}")
        ...
        try:
            ...
            # Create/get dataset
            dataset_id, created = pbi.get_or_create_push_dataset(dataset_name, table_name, schema)
            print(f"âœ… Dataset ID: {dataset_id} (Created: {created})")
            
            # Push data
            push_res = pbi.push_rows(dataset_id, table_name, rows)
            print(f"âœ… Data pushed successfully")
            
            # Apply measures
            measures_applied = {}
            if measures_list:
                print(f"ðŸ”§ Applying {len(measures_list)} measures...")
            ...
            try:
                ...
                print(f"âš ï¸ Refresh not available (expected for push datasets): {str(e)}")
            ...
            print(f"âŒ Power BI error: {str(e)}")
...
async def list_powerbi_datasets():
    ...
    try:
        ...
        print(f"âŒ Error listing datasets: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def download_pdf(payload: dict = Body(...)):
    ...
    try:
        ...
        elements.append(Paragraph("âœ“ Validation &amp; Reconciliation Report", title_style))
        ...
        if num_differences == 0:
            summary_text = f"<b style='color: green'>âœ“ SUCCESS:</b> Data synchronized perfectly. All {len(all_metrics)} metrics match."
        else:
            summary_text = f"<b style='color: red'>âš  ATTENTION:</b> {num_differences} difference(s) detected."
        ...
        for m in all_metrics:
            ...
            status_symbol = "âœ“ PASS" if m["status"] == "PASS" else "âœ— FAIL"
        ...
        for d in all_metrics:
            if d["metric"] == "total_records":
                variance_str = str(d["variance"]) if d["variance"] is not None else "â€”"
        ...
        if differences:
            ...
            for d in differences:
                ...
                variance_str = str(d["variance"]) if d["variance"] is not None else "â€”"
                variance_pct = f"{d['variance_percent']:.1f}%" if d["variance_percent"] is not None else "â€”"
        ...
        cert_status = "âœ“ CERTIFIED" if certified else "âœ— NOT CERTIFIED"
        ...
        print(f"âŒ Error downloading PDF: {str(e)}")
        import traceback
        traceback.print_exc()
...
# Wires loadscript_parser â†’ powerbi_dataset_publisher in one REST call.

# ==================== PARSE SCRIPT + CONVERT TO MQUERY ENDPOINTS ====================
# These are called by the frontend qlikApi functions:
#   parseLoadScript(script)          â†’ POST /api/parse-script
#   convertToMQuery(parsedScript, table_name) â†’ POST /api/convert-mquery
...
async def convert_mquery_endpoint(request: ConvertMQueryRequest):
    ...
    try:
        ...
        if not target:
            # Fuzzy match â€” case-insensitive
            target = next(
        ...
        return {
            ...
            "message":      (
                ...
                + (
                    f"âš ï¸ This RESIDENT table depends on '{target.get('source_path')}' â€” "
...
class PublishDatasetRequest(BaseModel):
    ...
    prefer_xmla:         bool = False            # default False â€” REST Push works on any workspace
...
async def publish_dataset_endpoint(
    ...
    """
    Full pipeline: fetch loadscript â†’ parse â†’ generate M expressions â†’ publish to Power BI.
    ...
    # â”€â”€ Resolve workspace_id â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Acquire access token via service principal (client_credentials) â”€â”€â”€â”€â”€
    ...
    if not access_token:
        try:
            ...
            logger.info("âœ… Power BI service principal token acquired")
        except HTTPException:
            raise
        except ImportError:
            raise HTTPException(
    ...
    # â”€â”€ 1. Get script â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if request.app_id:
    ...
    # â”€â”€ 3. Publish â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
