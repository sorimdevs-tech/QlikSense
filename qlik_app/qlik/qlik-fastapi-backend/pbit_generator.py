...
def parse_combined_mquery(combined_m: str) -> List[Dict[str, str]]:
    ...
    for i, match in enumerate(headers):
        ...
        let_match = re.search(r"(let\b.*?\bin\b\s+\S+)", chunk, re.DOTALL | re.IGNORECASE)
        if let_match:
...
def build_pbit(
    ...
    schema = {
        ...
        "model": {
            ...
            # DataSourcePath is exposed as a Power BI Query Parameter
            "expressions": [{
                "name": "DataSourcePath",
                "description": (
                ...
                "annotations": [{"name": "PBI_QueryOrder", "value": "0"}],
            }],
        },
    }
