...
# Ensure values in local `.env` take precedence over stale shell/session variables.
...
class QlikClient:
    def __init__(self, api_key: Optional[str] = None, tenant_url: Optional[str] = None):
        ...
        self.api_key = self._normalize_api_key(api_key or os.getenv("QLIK_API_KEY"))
    ...
    def _normalize_api_key(value: Optional[str]) -> str:
        """Normalize common key formats copied from docs/UI."""
