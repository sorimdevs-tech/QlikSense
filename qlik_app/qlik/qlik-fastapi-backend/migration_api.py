...
#     parsed_script_json: str = Body(..., description="Full parse result JSON from /parse-loadscript (send in POST body to avoid URL length limits)"),
...
#     (e.g. RESIDENT source tables that must also exist in the Power BI dataset).
...
#     logger.info("[convert_to_mquery_endpoint] Table: %s", table_name or "(all)")
#     logger.info("=" * 80)

#     # 1. Parse the JSON payload
#     try:
#         parse_result: Dict[str, Any] = json.loads(parsed_script_json)
...
# # ─────────────────────────────────────────────────────────────────────────────
# # SHAREPOINT FOLDER BROWSER — returns real subfolders inside Shared Documents
# # ─────────────────────────────────────────────────────────────────────────────
# @router.get("/sharepoint-folders")
# async def get_sharepoint_folders(
#     site_url: str = Query(..., description="SharePoint site URL e.g. https://company.sharepoint.com/sites/mysite"),
# ):
#     """
#     Fetch real subfolders from SharePoint Shared Documents library.
#     Uses Microsoft Graph API with Service Principal token.
#     Returns folder list so UI can show a dropdown for the user to choose.
#     """
#     import requests as _req
#     from urllib.parse import urlparse

#     site_url = site_url.strip().rstrip("/")
#     logger.info("[SP Folders] Fetching subfolders from: %s", site_url)

#     folders = []
#     error_msg = ""

#     # ── Method 1: Microsoft Graph API (most reliable) ──────────────────────
#     try:
#         from powerbi_publisher import _acquire_sp_token
#         graph_token = _acquire_sp_token("https://graph.microsoft.com/.default")
#         graph_headers = {
...
#         hostname = parsed.netloc                    # sorimtechnologies.sharepoint.com
#         site_path = parsed.path.rstrip("/")         # /sites/ddrive

#         # Step 1: Get the site ID
#         site_info_url = f"https://graph.microsoft.com/v1.0/sites/{hostname}:{site_path}"
#         si = _req.get(site_info_url, headers=graph_headers, timeout=15)
#         logger.info("[SP Folders] Site info: %d", si.status_code)

#         if si.ok:
#             site_id = si.json().get("id", "")
#             logger.info("[SP Folders] Site ID: %s", site_id)
...
#             dr = _req.get(drive_url, headers=graph_headers, timeout=15)
#             logger.info("[SP Folders] Drive root children: %d", dr.status_code)

#             if dr.ok:
#                 items = dr.json().get("value", [])
#                 for item in items:
#                     if "folder" in item:   # only folders, skip files
#                         name = item.get("name", "")
#                         if name and not name.startswith("."):
#                             folders.append({
#                                 "name": name,
#                                 "path": f"Shared Documents/{name}",
#                                 "item_id": item.get("id", ""),
#                             })
#                 logger.info("[SP Folders] Graph: found %d subfolder(s)", len(folders))
#             else:
#                 error_msg = f"Drive API {dr.status_code}: {dr.text[:200]}"
#         else:
#             error_msg = f"Site API {si.status_code}: {si.text[:200]}"

#     except Exception as ex:
#         error_msg = str(ex)
#         logger.warning("[SP Folders] Graph API error: %s", ex)

#     # ── Method 2: SharePoint REST API fallback ──────────────────────────────
#     if not folders:
#         try:
#             from powerbi_publisher import _acquire_sp_token
#             sp_token = _acquire_sp_token("https://sorimtechnologies.sharepoint.com/.default")
#             rest_headers = {
...
#                 f"('Shared%20Documents')/Folders"
#             )
#             r = _req.get(rest_url, headers=rest_headers, timeout=15)
#             logger.info("[SP Folders] REST API: %d", r.status_code)

#             if r.ok:
#                 results = r.json().get("d", {}).get("results", [])
#                 for item in results:
#                     name = item.get("Name", "")
#                     if name and not name.startswith(".") and name != "Forms":
#                         folders.append({
#                             "name": name,
#                             "path": f"Shared Documents/{name}",
#                             "item_id": "",
#                         })
#                 logger.info("[SP Folders] REST: found %d subfolder(s)", len(folders))
#             else:
#                 error_msg += f" | REST {r.status_code}"
#         except Exception as ex2:
#             error_msg += f" | REST error: {ex2}"
#             logger.warning("[SP Folders] REST fallback error: %s", ex2)

#     # Sort alphabetically for clean dropdown
...
#         "count": len(folders),
#         "message": (
#             f"Found {len(folders)} folder(s) in Shared Documents"
#             if folders
#             else f"No folders found — check permissions. Error: {error_msg}"
#         ),
#     }
...
class ConvertToMQueryBody(_BaseModel):
    """
    Request body for /convert-to-mquery.
    parsed_script_json: the full JSON string returned by /parse-loadscript
    """
...
@router.post("/convert-to-mquery")
async def convert_to_mquery_endpoint(
    body:              ConvertToMQueryBody,
    table_name:        str = Query("", description="Specific table to convert (empty = all tables)"),
    base_path:         str = Query("[DataSourcePath]", description="Base path for file sources"),
    connection_string: str = Query("", description="ODBC connection string for SQL sources"),
    sp_folder:         str = Query("", description="SharePoint subfolder name chosen by user (e.g. CSVFilesDatas)"),
):
    """
    Phase 6: Convert parsed LoadScript to Power Query M expressions.
    ...
    # 1. Parse the JSON payload (from Pydantic body)
    try:
        parse_result: Dict[str, Any] = json.loads(body.parsed_script_json)
    except json.JSONDecodeError as exc:
        raise HTTPException(status_code=400, detail=f"Invalid JSON in parsed_script_json: {exc}")
    ...
    # 2b. Inject sp_folder into every table's options so mquery_converter uses it
    # This is the folder the user selected in the Browse dropdown (e.g. "CSVFilesDatas")
    if sp_folder and sp_folder.strip():
        logger.info("[convert_to_mquery] Injecting sp_subfolder='%s' into %d table(s)", sp_folder, len(tables))
        for t in tables:
            opts = t.get("options", {})
            opts["sp_subfolder"] = sp_folder.strip()
            t["options"] = opts
