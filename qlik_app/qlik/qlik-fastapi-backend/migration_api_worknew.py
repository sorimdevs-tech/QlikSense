    ...
    try:
        ...
        try:
            ...
            if not parse_result or parse_result.get("status") == "error":
                ...
                logger.info(f"   - Tables: {parse_result.get('summary', {}).get('tables_count', 0)}")
                logger.info(f"   - Fields: {parse_result.get('summary', {}).get('fields_count', 0)}")
                # FIXED: always carry raw_script so SimpleMQueryGenerator and
                # RelationshipExtractor can read field lists and source paths.
        ...
        try:
            ...
            # FIXED: Always build generator with ALL tables so RelationshipExtractor
            # can detect cross-table joins. selected_table only filters the output.
            generator = create_mquery_generator(
                parsed_script=parse_result,
                table_name=None,          # load all tables for relationship detection
                lib_mapping=lib_mapping
            )
            # Now generate output for the requested table (or all if None)
            generator.selected_table = table_name
