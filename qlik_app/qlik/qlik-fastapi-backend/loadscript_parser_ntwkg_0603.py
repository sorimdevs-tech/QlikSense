...
def _parse_inline_block(inline_text: str) -> Tuple[List[Dict], Dict]:
    ...
    Handles:
    - Rows that wrap across multiple lines in the Qlik script
    - Quoted values containing commas  e.g.  "Chennai, TN"
    - Quoted values containing newlines (joins continuation lines)
    - Both single-quoted and unquoted values
    """
    import csv, io

    raw_lines = [l for l in inline_text.splitlines() if l.strip()]
    if not raw_lines:
        return [], {}

    # ── Step 1: re-join lines that are continuations of a wrapped row ─────────
    # A row is complete when its comma-count (outside quotes) matches
    # headers count - 1.  We detect incomplete rows by counting commas
    # outside quoted strings and joining with the next line until complete.

    def _count_commas_outside_quotes(s: str) -> int:
        count = 0
        in_q = False
        q_char = None
        for ch in s:
    ...
    n_headers = _count_commas_outside_quotes(header_line) + 1
    headers = [h.strip().strip('"').strip("'") for h in header_line.split(',')]

    # Re-join wrapped data rows
    joined_rows = []
    buf = ""
    for line in raw_lines[1:]:
        buf = (buf + " " + line).strip() if buf else line.strip()
        if _count_commas_outside_quotes(buf) >= n_headers - 1:
            joined_rows.append(buf)
            buf = ""
    if buf:
        joined_rows.append(buf)  # last partial row

    # ── Step 2: parse each joined row using csv reader for correct splitting ──
    sample_rows = []
    for row_str in joined_rows:
        # csv.reader handles quoted commas and embedded quotes correctly
        try:
            ...
            continue
        # Also try single-quote variant if double-quote parse gives wrong count
        ...
        vals = []
        for v in parsed:
            v = v.strip().strip("'").strip('"')   # strip Qlik single/double quotes
            v = v.replace("\r", "").replace("\n", " ")  # collapse any remaining newlines
    ...
    fields = [
        {'name': h, 'expression': h, 'alias': None, 'type': 'string',
    ...
    options = {
        ...
        'inline_row_count': len(joined_rows),
    }
