...
def _extract_fields_from_m(expr: str) -> list:
    """Extract column names from #table() type signature in M expression."""
    import re
    # Find content between "type table [" and "]"
    match = re.search(r"type\s+table\s+\[(.+?)\]", expr, re.DOTALL)
    if not match:
        logger.warning("[BIM] Could not extract fields from M expression")
        return []
...
def _sanitize_m(expr: str) -> str:
    """
    Make M safer for Fabric import:
    - Remove trailing commas before closing row lists (common cause of 'Token Literal expected').
    """
    import re
    # Remove trailing commas before any closing bracket
...
class _Publisher:
    ...
    def _build_bim(
        ...
        for t in tables_m:
            ...
            columns = []
            for f in fields:
                # Make the BIM column type lenient to avoid VT_BSTR -> VT_I8 errors on blanks.
                source_type = (t.get("source_type") or "").lower()
                raw_type = (f.get("type") or "").lower()
                if source_type in ("sql", "odbc") and raw_type in ("integer", "int64", "number", "double", "decimal"):
                    dtype = _tabular_type(raw_type)
                else:
                    dtype = "string"
        ...
        expressions = []
        # Always define DataSourcePath parameter so M expressions referencing [DataSourcePath]
        # are valid even when caller leaves the value blank.
        default_path = data_source_path or ""
        expressions.append({
            "name": "DataSourcePath",
            "kind": "m",
            "expression": [f'"{default_path}"']
        })
